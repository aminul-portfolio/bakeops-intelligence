...
If that line ever changes, the analysis has changed. The test suite and CI protect the metric-build workflow, and the reviewer command below verifies the signature output after a clean rebuild. The full repository, including the metric build code, the seed data definitions, the test suite, and the BI export contract, is at [GitHub repository](https://github.com/aminul-portfolio/bakeops-intelligence).
---
